"""Resume BTS KITTI full-run Session 3 under immutable 2xT4 Freeze v4."""
...
SESSION_CONFIG = WORKING / "session_3_config.txt"
HANDOFF = WORKING / "session_3_handoff"
STARTUP_RECEIPT = WORKING / "session_3_startup_audit.json"
START_RECEIPT = WORKING / "session_3_start_receipt.json"
BOUNDARY_RECEIPT = WORKING / "session_3_boundary_receipt.json"
SESSION_LOG = WORKING / "session_3_training.log"
...
SESSION = 3
INITIAL_STEP = 25000
FIRST_RESUMED_STEP = 25001
TARGET_RECOVERY_STEP = 65000
EXPECTED = {
    ...
    "initial_checkpoint": "2a5148f2d30636027b1b23287f4b8cc0439b3addd54408036947a7b179689fc5",
...
def startup_audit() -> dict:
    ...
    torch_home = Path("/tmp/bts_session_3_torch_home")
    ...
    result = {
        ...
        "freeze": "v4",
        "created_utc": now(),
        "status": "passed_before_resumed_optimizer_step_25001",
        "environment": {
...
def launch() -> dict:
    ...
    with SESSION_LOG.open("w", encoding="utf-8") as log:
        ...
        for line in process.stdout:
            ...
            log.write(line)
            log.flush()
            if re.search(r"\[4\]\[1841/5790/25001\]", line) or re.search(r"25001", line):
    ...
    if not START_RECEIPT.is_file():
        raise RuntimeError("Step-25001 start receipt was not emitted")
    if not recovery.is_file():
