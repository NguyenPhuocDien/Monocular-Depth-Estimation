"""Launch fresh BTS KITTI Run 02 Session 1 under immutable 2xT4 Freeze v4."""
...
def startup_audit() -> dict:
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") in {"0", "1"}:
        raise RuntimeError("Both T4 devices must remain visible")
    if len(effective) != 2 or any("T4" not in name for name in effective):
        raise RuntimeError(f"Expected two effective T4 GPUs, got {effective}")
    if len(physical) != 2 or any("T4" not in name for name in physical):
        raise RuntimeError(f"Expected two physical T4 GPUs, got {physical}")
    ...
    if receipt["gate_state"]["session_1"] != "authorized_not_started":
        raise RuntimeError("Freeze v4 does not authorize Session 1")
    if receipt["artifacts"]["full_run_manifest"]["sha256"] != EXPECTED["freeze_manifest"]:
        raise RuntimeError("Freeze receipt manifest hash mismatch")
    if receipt["artifacts"]["checkpoint_retention_plan"]["sha256"] != EXPECTED["freeze_retention"]:
    ...
    if f"--model_name {MODEL_NAME}" not in config_text:
        raise RuntimeError("Launcher model directory does not match frozen config")
    ...
    with SESSION_LOG.open("w", encoding="utf-8") as log:
        ...
        for line in process.stdout:
            ...
            if match:
                ...
                if candidate["optimizer_state_entries"] != 227:
