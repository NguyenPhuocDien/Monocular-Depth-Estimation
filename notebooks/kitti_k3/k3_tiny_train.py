"""K3: three-step BTS KITTI tiny-train gate without checkpointing."""
...
OUTPUT = Path("/kaggle/working/k3_tiny_train_report.json")
...
def install_torch_weight(source: Path) -> Path:
    torch_home = Path("/tmp/bts_k3_torch_home")
...
def run() -> dict:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("K3 requires a Kaggle GPU")
    ...
    args = SimpleNamespace(
        ...
        filenames_file=str(train_list), input_height=352, input_width=704,
        batch_size=1, num_threads=1, distributed=False, use_right=False,
        do_kb_crop=True, do_random_rotate=True, degree=1.0,
    )
    ...
    criterion = silog_loss(variance_focus=0.85)
    tracked_name, tracked_parameter = next(
        (name, parameter) for name, parameter in model.named_parameters()
        if name.startswith("decoder.") and parameter.requires_grad
    )
    ...
    for _ in range(TINY_STEPS):
        ...
        gradient_checks.append(bool(gradients) and all(bool(torch.isfinite(g).all()) for g in gradients))
        optimizer.step()
        losses.append(float(loss.detach().cpu()))
        valid_pixels.append(int(mask.sum()))

    tracked_delta = float((tracked_parameter.detach() - tracked_before).abs().max().cpu())
    checks = {
        ...
        "optimizer_is_adamw": isinstance(optimizer, torch.optim.AdamW),
        "optimizer_group_weight_decay": [g["weight_decay"] for g in optimizer.param_groups] == [1e-2, 0.0],
        "optimizer_lr": all(g["lr"] == 1e-4 for g in optimizer.param_groups),
        "optimizer_eps": all(g["eps"] == 1e-3 for g in optimizer.param_groups),
        "exact_tiny_steps": len(losses) == TINY_STEPS,
        "all_losses_finite_positive": all(np.isfinite(value) and value > 0 for value in losses),
        "all_batches_have_valid_depth": all(value > 0 for value in valid_pixels),
        "all_gradients_finite": all(gradient_checks),
        "parameter_updated": tracked_delta > 0,
    }
    return {
        "gate": "K3",
...
if __name__ == "__main__":
    try:
        ...
        report = {
            "gate": "K3", "status": "failed",
            "completed_utc": datetime.now(timezone.utc).isoformat(),
            "error": f"{type(exc).__name__}: {exc}",
            "traceback": traceback.format_exc(),
            "authorization_boundary": {
                "checkpoint_written": False, "resume_executed": False,
