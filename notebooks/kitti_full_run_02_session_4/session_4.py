"""Resume BTS KITTI full-run Session 4 under immutable 2xT4 Freeze v4."""
...
SESSION_CONFIG = WORKING / "session_4_config.txt"
HANDOFF = WORKING / "session_4_handoff"
STARTUP_RECEIPT = WORKING / "session_4_startup_audit.json"
START_RECEIPT = WORKING / "session_4_start_receipt.json"
BOUNDARY_RECEIPT = WORKING / "session_4_boundary_receipt.json"
SESSION_LOG = WORKING / "session_4_training.log"
...
SESSION = 4
INITIAL_STEP = 65000
FIRST_RESUMED_STEP = 65001
TARGET_RECOVERY_STEP = 105000
EXPECTED = {
    ...
    "initial_checkpoint": "c605a0a1cd3bbf40e3bbf70561c500673b344bf0c5b6eace345912c62e4caa1e",
...
def startup_audit() -> dict:
    ...
    torch_home = Path("/tmp/bts_session_4_torch_home")
    ...
    result = {
        ...
        "freeze": "v4",
        "created_utc": now(),
        "status": "passed_before_resumed_optimizer_step_65001",
        "environment": {
...
def launch() -> dict:
    ...
    if not START_RECEIPT.is_file():
        raise RuntimeError("Step-65001 start receipt was not emitted")
    if not recovery.is_file():
