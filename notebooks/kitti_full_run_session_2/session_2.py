"""Resume BTS KITTI full-run Session 2 under immutable Freeze v3."""
...
EXPECTED = {
    "freeze_v2_manifest": "c770cdeebfb0515347e868dbcd810be054bd235a49589aa5bb3184ac4990716a",
    "freeze_v2_retention": "e7fd7174f3a8132c6787b837f7cb686dfad2c6210624bcc0b7a7c1d10630be86",
    "freeze_v2_receipt": "f775dd007721bd5fb8fda5b6203de43d1e08dbfdadacbc592f18b98e8ba5c60a",
    "freeze_v3_manifest": "84490aab23881b2835e588ae6f1c4d946280aac4e424d73bd4fe64559aa141ae",
    "freeze_v3_retention": "784b79311789b70b163767303a2877b7dbba0aa433193d37188d2c858525c550",
    "freeze_v3_receipt": "8acbf5bef6225054277f00cc9e9387d12488ca541dffeb3722f10367250ce061",
    "checkpoint_audit": "127aa717ec98ab264ccb6db5a2e555eec8c9c9bf6f81d6067afd328402fea065",
    "initial_checkpoint": "35a36b5f7b4d972b34b6ef850237174f3c1b8d3d65fdc96f4e5918cf6f3ae579",
...
def startup_audit() -> dict:
    ...
    contract_paths = {
        "freeze_v2_manifest": find_one("full_run_manifest.v2.json", EXPECTED["freeze_v2_manifest"]),
        "freeze_v2_retention": find_one("checkpoint_retention_plan.v2.json", EXPECTED["freeze_v2_retention"]),
        "freeze_v2_receipt": find_one("freeze_receipt.v2.json", EXPECTED["freeze_v2_receipt"]),
        "freeze_v3_manifest": find_one("full_run_manifest.v3.json", EXPECTED["freeze_v3_manifest"]),
        "freeze_v3_retention": find_one("checkpoint_retention_plan.v3.json", EXPECTED["freeze_v3_retention"]),
        "freeze_v3_receipt": find_one("freeze_receipt.v3.json", EXPECTED["freeze_v3_receipt"]),
        "checkpoint_audit": find_one("bts_kitti_step500_checkpoint_contract.json", EXPECTED["checkpoint_audit"]),
    }
    receipt = json.loads(contract_paths["freeze_v3_receipt"].read_text())
    manifest = json.loads(contract_paths["freeze_v3_manifest"].read_text())
    if receipt["gate_state"]["session_2"] != "authorized_not_started":
        raise RuntimeError("Freeze v3 does not authorize Session 2")
    if manifest["base_contract"]["sha256"] != EXPECTED["freeze_v2_manifest"]:
        raise RuntimeError("Freeze v3 base contract mismatch")
    if manifest["resume_contract"]["checkpoint_sha256"] != EXPECTED["initial_checkpoint"]:
        raise RuntimeError("Freeze v3 checkpoint lineage mismatch")
...
def create_start_receipt() -> None:
    if START_RECEIPT.exists():
        return
    write_json(START_RECEIPT, {
        "run_id": RUN_ID,
        "session": SESSION,
        "freeze": "v3",
        "created_utc": now(),
        "initial_checkpoint_sha256": EXPECTED["initial_checkpoint"],
...
def launch() -> dict:
    ...
    with SESSION_LOG.open("w", encoding="utf-8") as log:
        ...
        for line in process.stdout:
            ...
            if "Saved verified recovery checkpoint:" in line and recovery.is_file():
                ...
                if step <= INITIAL_STEP or step % 500 != 0:
                    raise RuntimeError(f"Unexpected recovery step: {step}")
                if step == TARGET_RECOVERY_STEP:
                    boundary_checkpoint = candidate
                    os.killpg(process.pid, signal.SIGTERM)
                    break
                if step > TARGET_RECOVERY_STEP:
                    raise RuntimeError(f"Training passed target boundary: {step}")
    try:
    ...
    if boundary_checkpoint is None:
        raise RuntimeError(
            f"Training exited before verified step-{TARGET_RECOVERY_STEP} recovery: {returncode}"
        )
    ...
    if len(best_candidates) > 1:
        raise RuntimeError(f"Expected at most one current best abs_rel checkpoint: {best_candidates}")
    if best_candidates:
    ...
    for path in sorted(model_dir.glob("model-*-best_*")):
        item = verify_checkpoint(path)
        item["source_name"] = path.name
        best_ledger.append(item)
    ...
    boundary = {
        ...
        "latest": latest,
        "best_abs_rel_emitted_this_session": best is not None,
        "best_abs_rel": best,
        "inherited_best_abs_rel": 0.11019 if best is None else None,
        "inherited_best_abs_rel_sha256": (
            "820e9ea6328c1fc09fff54b30976da98e7e0e7d4ba65b8650609d9820c9be291"
            if best is None else None
        ),
    ...
    artifacts = [
        ...
        SESSION_LOG.name,
        "session_2_handoff/latest.pth",
        "session_2_handoff/handoff_manifest.json",
        "session_2_handoff/best_metric_evidence.json",
    ]
    if best is not None:
        artifacts.append("session_2_handoff/best_abs_rel.pth")
    write_json(ARTIFACTS, {
        "run_id": RUN_ID,
        "session": SESSION,
        "artifacts": artifacts,
    })
    shutil.rmtree(WORKING_RUNTIME)
...
if __name__ == "__main__":
    ...
    if result["status"] != "boundary_checkpoint_verified_local_output_pending_remote_handoff":
        raise SystemExit(1)
